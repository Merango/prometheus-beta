...
def convert_to_alternating_pascal_case(input_string: str) -> str:
    ...
    for word in words:
        ...
        for i in range(1, len(converted_word), 2):
            converted_word[i] = converted_word[i].lower()
        for i in range(0, len(converted_word), 2):
            converted_word[i] = converted_word[i].upper()
